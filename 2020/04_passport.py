...
def parse_input(data):
    ...
    items = [[j for j in i if j] for i in items]
    items = [[j.split(":") for j in i] for i in items]
    items = [[j for j in i if j != [""]] for i in items]
...
def part_one():
    ...
    not_required = ["cid"]
    valid = sum(all(key in item.keys() for key in required_keys) for item in parsed)
...
def part_two():
    ...
    valid = 0
    for i in parsed:
        if any(key not in i.keys() for key in required_keys):
            continue
        all_valid = all(checks[key](value) for key, value in i.items())
        if all_valid:
            valid += 1
    print(valid)
